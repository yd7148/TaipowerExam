"""Verify the generated 103 (電機(甲)) full-answer report against the authoritative key.

Checks for every question: section header answer, 題目/答案選項/實際答案/解題過程
presence, and agreement with answers_103.json. Results saved to a separate file.
"""
import re
import json
import os
...
REPORT = os.path.join(BASE, '完整詳細解答- 103 年經濟部所屬事業機構新進職員甄試試題（電機(甲)）.md')
QDIR = os.path.join(BASE, 'test-pdf', '103-2014', '電機(甲)', '提取結果_v103a')
OUT = os.path.join(BASE, '驗證結果- 103 年經濟部所屬事業機構新進職員甄試試題（電機(甲)）.md')
N = 40
...
for i in range(1, len(sections), 2):
    ...
    if '或' in str(off):
        header_ok = header_ans.startswith(str(off))
    else:
        header_ok = (header_ans == off) or (header_ans.startswith(off) and off in header_ans) or ('送分' in str(off) and header_ans == '送分')
    if not header_ok:
...
# ensure all N questions are represented
missing = [q for q in range(1, N + 1) if q not in [r['qid'] for r in rows]]
for q in missing:
    rows.append({'qid': q, 'header_ans': '?', 'official': official.get(q, '?'), 'question': False, 'options': False, 'solution': False, 'header_ok': False})

bad = [r for r in rows if not (r['question'] and r['options'] and r['solution'] and r['header_ok'])]

lines = []
lines.append("# 驗證結果 — 103 年經濟部所屬事業機構新進職員甄試試題（電機(甲) 完整詳細解答）")
lines.append("")
lines.append(f"- 檢查時間：今日")
lines.append(f"- 受測檔案：`{os.path.basename(REPORT)}`")
lines.append(f"- 比對基準：`answers_103.json`（官方解答 PDF 解析之答案鍵）")
...
print(f"checked {len([r for r in rows if r['qid']<=N])} questions, failed={len(bad)}")
if bad:
    for r in bad:
