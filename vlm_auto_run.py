...
def load_cfg(year):
    global CFG, CIRCUIT_QUESTIONS, ANSWER_LETTERS, ANSWER_VALUES, _IMAGE_BASE
    here = os.path.dirname(os.path.abspath(__file__))
    cfg_path = None
    for cand in (os.path.join(here, f'vlm_cfg_{year}.json'),
                 os.path.join(here, 'scripts_extract_old', f'vlm_cfg_{year}.json')):
        if os.path.exists(cand):
            cfg_path = cand
            break
    if cfg_path is None:
        raise SystemExit(f"cfg not found for {year}")
    with open(cfg_path, encoding='utf-8') as f:
        CFG = json.load(f)
    _IMAGE_BASE = CFG['image_base']
    CIRCUIT_QUESTIONS = [int(q) for q in CFG['circuit_questions']]
    ANSWER_LETTERS = {int(k): v for k, v in CFG['answer_letters'].items()}
    ANSWER_VALUES = {int(k): v for k, v in CFG.get('answer_values', {}).items()}
...
def load_question_text(qid: int) -> str:
    ...
    md_path = os.path.join(_IMAGE_BASE, f"q{qid:02d}.md")
...
def main():
    ...
    if '--cfg' not in args:
        raise SystemExit("usage: python vlm_auto_run.py --cfg <year> [--question N] [--verbose]")
    year = args[args.index('--cfg') + 1]
    load_cfg(year)
    report_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'vlm_auto_report_{year}.json')
    only_q = None
    if '--question' in args:
    ...
    for qid in question_ids:
        img_path = os.path.join(_IMAGE_BASE, f"q{qid:02d}.png")
        if not os.path.exists(img_path):
            print(f"\n  Q{qid}: image not found")
            continue

        print(f"\n  --- Q{qid}: {CFG.get('question_text', {}).get(str(qid), '')[:40]} ---")
