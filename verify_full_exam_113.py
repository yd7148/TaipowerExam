...
presence, and agreement with answers_113.json. Results saved to a separate file.
"""
import re
import json
import os
...
REPORT = os.path.join(BASE, '完整詳細解答- 113 年經濟部所屬事業機構新進職員甄試試題.md')
QDIR = os.path.join(BASE, 'test-pdf', '113-2024', '電機', '提取結果_v113')
OUT = os.path.join(BASE, '驗證結果- 113 年經濟部所屬事業機構新進職員甄試試題.md')
...
lines = []
lines.append("# 驗證結果 — 113 年經濟部所屬事業機構新進職員甄試試題（完整詳細解答）")
lines.append("")
lines.append(f"- 檢查時間：今日")
lines.append(f"- 受測檔案：`{os.path.basename(REPORT)}`")
lines.append(f"- 比對基準：`answers_113.json`（官方解答 PDF 解析之答案鍵）")
