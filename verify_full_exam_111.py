"""Verify the generated 111 full-answer report against the authoritative key.

Checks for every question: section header answer, 題目/答案選項/實際答案/解題過程
presence, and agreement with answers_111.json. Results saved to a separate file.
"""
import re
import json
import os
...
REPORT = os.path.join(BASE, '完整詳細解答- 111 年經濟部所屬事業機構新進職員甄試試題.md')
QDIR = os.path.join(BASE, 'test-pdf', '111-2022', '電機(一)', '提取結果_v111')
OUT = os.path.join(BASE, '驗證結果- 111 年經濟部所屬事業機構新進職員甄試試題.md')
...
lines = []
lines.append("# 驗證結果 — 111 年經濟部所屬事業機構新進職員甄試試題（完整詳細解答）")
lines.append("")
lines.append(f"- 檢查時間：今日")
lines.append(f"- 受測檔案：`{os.path.basename(REPORT)}`")
lines.append(f"- 比對基準：`answers_111.json`（官方解答 PDF 解析之答案鍵）")
